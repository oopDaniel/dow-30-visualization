...
class Chart extends Component {
  ...
  renderChart() {
    ...
    enter.append('rect')
      .attr('class', styles.theme_unchanged)
      .attr('x', (d, i) => this.state.scale.x(i + 1) - d3Params.barWidth / 2)
      .attr('y', d => {
        console.warn(height - d3Params.paddingBottom * height);
        return  height - d3Params.paddingBottom * height
      })
      // .attr('fill', 'steelblue')
    ...
    .transition()
      ...
      .attr('y', d => {
        console.warn(d3Params.paddingBottom * height, this.state.scale.y(d.prevValue), this.state.scale.y(d.prevValue) - d3Params.paddingBottom * height)
        console.warn(height - this.state.scale.y(d.prevValue))
        return this.state.scale.y(d.prevValue) - d3Params.paddingBottom * height
      })
      .attr('height', d => height - this.state.scale.y(d.prevValue))
...
///////////////////////////////
    enter.append('rect')
      ...
      .attr('height', 0)
    .transition()
      .delay((d, i) => (i + 1) * d3Params.delay)
      .attr('y', d => d.trend === 1
          ? this.state.scale.y(d.value)*.9 - d3Params.paddingBottom * height
          : this.state.scale.y(d.prevValue) - d3Params.paddingBottom * height)
      .attr('height', d => {
        console.error(d.value, this.state.scale.y(d.value));
        console.error(d.prevValue, this.state.scale.y(d.prevValue));
        console.error(this.state.scale.y(d.value - d.prevValue));
        // return 15;
        // return height - this.state.scale.y(d.prevValue)
        return (d.trend === 1
            ? this.state.scale.y(d.prevValue) - this.state.scale.y(d.value)
            : this.state.scale.y(d.value) - this.state.scale.y(d.prevValue)
            ) * 10
      })
      .duration(d3Params.enterDuration)
      .call(this.state.hook);
